...
auto *listener = new JsonTestListener("test_customer.json");

class CustomerClassificationTest : public ::testing::Test
{
public:
...
std::string filename = "resource/old/customer.csv";
Dataset CustomerClassificationTest::dataset = Dataset::readFromCsv(filename);
std::pair<Dataset, Dataset> CustomerClassificationTest::datasetPair = dataset.splitDatasetOnTrainAndTest(0.8);

TEST_F(CustomerClassificationTest, BasicClassification)
{
    const int m = 11;
    const int epochs = 5;
    ...
    listener->AddTestDetails(
        "CustomerClassificationTest.BasicClassification",
        {{"parameters", {{"epochs", epochs}, {"clusters", m}}},
         {"results", results}});
}

TEST_F(CustomerClassificationTest, NoisyDataClassification)
{
    const std::vector<int> clusterCounts = {8, 9, 10, 11, 12, 13, 14, 15};
    ...
    listener->AddTestDetails(
        "CustomerClassificationTest.NoisyDataClassification",
        {{"cluster_experiments", noise_results}});
}

TEST_F(CustomerClassificationTest, DifferentClusterCounts)
{
    const std::vector<int> clusterCounts = {8, 9, 10, 11, 12, 13, 14, 15};
    int epochs = 10;
    ...
    listener->AddTestDetails(
        "CustomerClassificationTest.DifferentClusterCounts",
