...
void tsk::TSK::print(std::ostream &os) const
{
    // Получаем ссылки на параметры
    const auto &sigma = _fuzzyLayer.sigma;
    const auto &c = _fuzzyLayer.c;
    const auto &b = _fuzzyLayer.b;
    const auto &p = _multipleLayer.p;

    // Выводим параметры c, b, sigma
    os << "Fuzzy Layer Parameters:\n";
    os << "c: [";
...
metric::Metric tsk::TSK::evaluate(const boost::multi_array<double, 2> &x, const std::vector<double> &y, int classesCount) const
{
    /**
     * x - входные векторы
    ...
    double accuracy = metric::Metric::calculateAccuracy(y, predictedValues, classesCount);
    double mse = metric::Metric::calculateMSE(y, predictedValues, classesCount);

    metric::Metric metrics{accuracy, mse};
    return metrics;
}